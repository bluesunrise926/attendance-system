// ============================================================
// 員工打卡系統 — Firebase 整合版主程式（All-in-one，不依賴外部 config）
// 符合台灣勞動基準法第30條出勤記錄規定
// ============================================================
...
function resetIdleTimer() {
  ...
  warnTimer = setTimeout(() => showAutoLogoutBar(), IDLE_TIMEOUT - WARN_BEFORE);
...
async function loadUserData(uid) {
  try {
    ...
    currentUserData = snap.exists()
      ? { uid, ...snap.data() }
      : { uid, name: '用戶', role: 'employee', dept: '' };
  } catch (e) {
  ...
  try {
    if (type === 'in') {
      await setDoc(doc(db, 'records', recId), {
        empId: currentUser.uid, empName: currentUserData.name,
        empDept: currentUserData.dept || '', date: today,
        clockIn: timeStr, clockOut: null,
      ...
      await updateDoc(doc(db, 'records', recId), { clockOut: timeStr, clockOutAt: serverTimestamp() });
...
async function loadRecords() {
  ...
  records.forEach(r => {
    ...
    html += `<tr><td>${r.date}</td><td>${r.empName||''}</td><td>${r.empDept||''}</td>
      <td>${r.clockIn||'--'}</td><td>${r.clockOut||'--'}</td><td>${h}</td>
      <td style="font-size:12px;color:#999;">${loc}</td><td>${r.note||''}</td></tr>`;
  });
...
async function loadEmployeeList() {
  ...
  employees.forEach(e => {
    html += `<tr>
      <td><strong>${e.name}</strong></td><td>${e.email||''}</td><td>${e.dept||''}</td>
      <td>${e.joinDate||''}</td>
...
async function createEmployee() {
  ...
  try {
    ...
    await setDoc(doc(db, 'users', cred.user.uid), {
      name, email, dept, joinDate, role: 'employee', active: true, createdAt: serverTimestamp()
    });
    showToast(`員工 ${name} 建立成功`, 'success');
    closeModal('addEmpModal');
    loadEmployeeList();
    // 重新登入管理員
    if (currentUser) await signInWithEmailAndPassword(auth, currentUser.email, '');
  } catch (e) {
...
async function exportWorkStatsCSV() {
  ...
  const employees = empSnap.docs.map(d => ({ id: d.id, ...d.data() })).filter(e => e.role === 'employee');
  const q = query(collection(db, 'records'), where('date', '>=', month+'-01'), where('date', '<=', month+'-31'));
